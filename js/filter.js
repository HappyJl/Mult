document.addEventListener('DOMContentLoaded',evt => {
    ...
    bat.addEventListener('click',ev => {
        ...
        let request = fetch('../php/filtter_api.php', params);
        request.then(value => {
...
function createNewMult(row){
    ...
    linkPage.href = 'mult.php?id=' + row['Id мультфильма'];
    let image = document.createElement('img');
    image.src = '../img/mult/' + row['Изображение'];
    ...
    opis.innerText = row['Название'];

    let opis2 = document.createElement('div')
    opis2.className = 'mult_opis2';
    opis2.innerText = row['Год выпуска'];

    container.appendChild(opis);
    container.appendChild(opis2);
